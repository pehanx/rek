...
var succesSendInputforData;
$(function(){
	succesSendInputforData = function popupok(){
		$('.popupsucces').addClass('popupsucces-active');
		setTimeout(function(){
			$('.popupsucces').removeClass('popupsucces-active');
		},2000);
	}
	...
	$('.placeholder .textup').each(function(){
		...
		$(this).on('focusout', function(){
			var elem = $(this);
			setTimeout(function(){
				if(elem.val().replace(/\s/g, '').length>0 && !elem.hasClass('input-forpass')){
					elem.parent().addClass('placeholder-active');
				}else if(!elem.hasClass('input-forpass')){
					elem.parent().removeClass('placeholder-active');
					elem.val('');
				}
			},200);
			if(elem.val().replace(/\s/g, '').length>0 && elem.hasClass('input-forpass')){
	...
	$('.placeholder .input-forpass').each(function(){
		$(this).on('focus', function(){
			$(this).parent().find('div').addClass('password-eye');
		});
		$(this).on('focusout', function(){
			$(this).parent().find('div').removeClass('password-eye');
		});
		$(this).parent().find('.eye').on('click',function(e){
				$(this).removeClass('eye_active');
				$(this).parent().find('.eyenot').addClass('eye_active');
				$(this).parent().find('.input').attr('type','password');
		});
		$(this).parent().find('.eyenot').on('click',function(e){
			$(this).removeClass('eye_active');
			$(this).parent().find('.eye').addClass('eye_active');
			$(this).parent().find('.input').attr('type','text');
		});
	});
	$('.popupbg').on('click',function(){
		$('.popup').removeClass('popupopacity');
		$(this).removeClass('popupbgopacity');
		setTimeout(function(){
			$('.popup').removeClass('popupblock');
			$('.popupbg').removeClass('popupblock');
	...
	$('.popupjoin').on('click',function(){
		$('.popupjoinform').removeClass('popupopacity');
		$(this).removeClass('popupbgopacity');
		setTimeout(function(){
	...
	$('.popupjoinform').on('click',function(e){
		e.stopPropagation()
	})
	$('.popupopen').on('click',function(e){
		...
		$('.popup').addClass('popupblock');
		$('.popupbg').addClass('popupblock');
		$('body, html').addClass('noscroll');
		$('body').width(width);
		setTimeout(function(){
			$('.popup').addClass('popupopacity');
			$('.popupbg').addClass('popupbgopacity');
		},200);
	});
	$('.passwordone, .passwordtwo').on("propertychange change click keyup input paste",function(){
		setTimeout(function(){
			if($('.passwordone').val()==$('.passwordtwo').val() && $('.passwordone').val().length>0 && $('.passwordtwo').val().length>0){
				$('.succes').addClass('succes-ok');
			}else{
		...
		if(error == 0){
			...
			$.ajax({
				...
				success: function (response) {
					...
					$('.succes').removeClass('succes-ok');
					$('.popup').removeClass('popupopacity');
					$('.popupbg').removeClass('popupbgopacity');
					$('.popupjoinform').removeClass('popupopacity');
					$('.popupjoin').removeClass('popupbgopacity');
					setTimeout(function(){
						$('.popup').removeClass('popupblock');
						$('.popupbg').removeClass('popupblock');
						$('.popupjoinform').removeClass('popupblock');
	...
	if($(window).width()>920){
			...
			$('.mobile-menu-clouse').removeClass('mobile-menu-clouse-active');
	...
	$('.mobile-menu').on('click',function(e){
		...
		$('.mobile-menu-clouse').addClass('mobile-menu-clouse-active');
		$('.dropdown').addClass('dropdown-open');
	});
	$('.mobile-menu-clouse').on('click',function(e){
		e.preventDefault();
		$(this).removeClass('mobile-menu-clouse-active');
		$('.mobile-menu').addClass('mobile-menu-active');
		$('.dropdown').removeClass('dropdown-open');
	});
	if($('.status-scrollbar').length){
		var heightDocument = $(document).height() - $(window).height();
		var heightScroll = $(window).scrollTop(); 
		var status = (heightScroll * 100)/heightDocument;
		$('.status-scrollbar').width(status+"%");
		$(window).scroll(function(){
			var heightDocument = $(document).height() - $(window).height();
			var heightScroll =  $(window).scrollTop(); 
			var status = (heightScroll * 100)/heightDocument;
	...
	$('.popup-join-open').on('click',function(e){
		...
		$('.popupjoin').addClass('popupblock');
		$('body, html').addClass('noscroll');
		$('body').width(width);
		setTimeout(function(){
			$('.popupjoinform').addClass('popupopacity');
			$('.popupjoin').addClass('popupbgopacity');
		},200);
	});
	$('.set-back-img').each(function(){
		var src = $(this).find('.get-back-img').attr('src');
